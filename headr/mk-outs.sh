...
for FILE in $INPUTS/*.txt; do
    ...
    head -n -0  $FILE > ${OUT_DIR}/${BASENAME}.n-0.out
    head -n -1  $FILE > ${OUT_DIR}/${BASENAME}.n-1.out
    head -n -10 $FILE > ${OUT_DIR}/${BASENAME}.n-10.out
...
head -n -0  $ALL > $OUT_DIR/all.n-0.out
head -n -1  $ALL > $OUT_DIR/all.n-1.out
head -n -10 $ALL > $OUT_DIR/all.n-10.out
